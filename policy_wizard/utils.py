...
def role_identifier(ext_roles_text):
    ...
    if not bool(context_roles):#if context_roles list is empty, i.e. if there are no context roles...
        ...
        elif ('Learner' in context_roles_as_string) or ('NonCreditLearner' in context_roles_as_string) or ('Mentor' in context_roles_as_string) or ('ContentDeveloper' in context_roles_as_string):
            policy_role = 'Student'
        else:
            raise PermissionDenied
